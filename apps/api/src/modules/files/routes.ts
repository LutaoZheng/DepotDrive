...
export async function fileRoutes(app:FastifyInstance){app.addHook('preHandler',authenticate);
 app.post('/upload',async(req,reply)=>{const part=await req.file();if(!part)return fail(reply,400,'FILE_REQUIRED','A file is required');
   ...
   const placements=await app.replicas.replicateFrom(app.storage,storageKey).catch(async error=>{await app.storage.delete(storageKey).catch(()=>undefined);throw error});
   try{const file=await prisma.file.create({data:{ownerId:req.user.sub,folderId,name:parsedName.data,originalName:parsedName.data,mimeType:part.mimetype||'application/octet-stream',sizeBytes:BigInt(stored.sizeBytes),storageKey,checksum:stored.checksum,replicas:{create:app.storageMetadata.replicaCreateData(placements)}}});await app.storage.delete(storageKey).catch(()=>undefined);return reply.code(201).send(fileDto(file));}
   catch(e){await app.replicas.deletePlacements(placements);await app.storage.delete(storageKey).catch(err=>req.log.error(err,'Failed upload compensation'));throw e;}
 });
 app.get('/:fileId/download',async(req,reply)=>{const p=params.safeParse(req.params);if(!p.success)return fail(reply,400,'VALIDATION_ERROR','Invalid file id');const file=await owned(p.data.fileId,req.user.sub);if(!file)return fail(reply,404,'FILE_NOT_FOUND','File not found');const replicas=await app.replicas.replicasForFile(file.id);let stream;if(replicas.length)stream=(await app.replicas.openDownload(replicas,app.config.STORAGE_FAILURE_TIMEOUT_MS)).stream;else{if(!await app.storage.exists(file.storageKey))return fail(reply,404,'FILE_CONTENT_MISSING','File content is missing');stream=app.storage.createReadStream(file.storageKey)}reply.header('Content-Type',file.mimeType).header('Content-Length',file.sizeBytes.toString()).header('Content-Disposition',contentDisposition(file.name));return reply.send(stream);});
 ...
 app.delete('/:fileId',async(req,reply)=>{const p=params.safeParse(req.params);if(!p.success)return fail(reply,400,'VALIDATION_ERROR','Invalid file id');const file=await owned(p.data.fileId,req.user.sub);if(!file)return fail(reply,404,'FILE_NOT_FOUND','File not found');
   // Replica deletion is best-effort and idempotent; metadata deletion must remain possible if a node is already unavailable.
   const replicas=await app.replicas.replicasForFile(file.id);if(replicas.length)await app.replicas.deleteFileReplicas(replicas);else await app.storage.delete(file.storageKey);await prisma.file.delete({where:{id:file.id}});return reply.code(204).send();});
}
